...
void time_synch_process()
{
	...
	{
		/*read network time, Ling Wang, 12/1/2017*/
		//memcpy(&network_time, buf_ptr, sizeof(rtimer_clock_t));
		//rtimer_clock_t network_time;
		memcpy(&slot_number, buf_ptr, sizeof(uint32_t));
		buf_ptr += sizeof(uint32_t);
		memcpy(&clock_time, buf_ptr, sizeof(rtimer_clock_t));
		...
		network_time_set(slot_number, clock_time);
		/*Synchronize netwrok time, Ling Wang, 12/2/2017*/
		current_slot_start = slot_number;
		ASN_INIT(current_asn, 0, slot_number);
	...
	{
		/*Check if synchronized, Ling Wang, 12/1/2017*/
