...
static uint32_t network_time_slot = 0; // Most significant bits of the current time.
 ...
 * @retval  None
 */
 /*network time = 32 bit time slot number + 16 bit clock time(one time slot)*/
void network_time_set(uint32_t slot_number, rtimer_clock_t clock_time) {
	network_time_slot = slot_number;
	TIM_SetCounter(TIM3, (uint16_t)clock_time);
}

uint32_t network_timeslot_now()
{
	return network_time_slot;
}
/*
...
void TIM3_IRQHandler(void)
{
    if(TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET) {
	    ...
	    network_time_slot++;
        //++network_time_msb;
	    /*Trigger scheduling, Ling Wang, 11/29/2017*/
	    process_poll(&rtimer_process);
    } 
	if(TIM_GetITStatus(TIM3, TIM_IT_CC1) != RESET) {
        TIM_ClearITPendingBit(TIM3, TIM_IT_CC1);
		if (network_time_slot == target_time_msb)
		{
...
void rtimer_arch_init(void) {
	...
	/*Each timer set to 20ms, Ling Wang, 11/29/2017*/
