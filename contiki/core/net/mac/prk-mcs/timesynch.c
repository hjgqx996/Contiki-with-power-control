...
void time_synch_send()
{
	...
	/*change network, Ling Wang, 12/1/2017*/
	//rtimer_clock_t network_time = RTIMER_NOW();
	uint32_t slot_number = network_timeslot_now();
	memcpy(buf_ptr, &slot_number, sizeof(uint32_t));
	buf_ptr += sizeof(uint32_t);
	rtimer_clock_t clock_time = RTIMER_NOW();
	memcpy(buf_ptr, &clock_time, sizeof(rtimer_clock_t));
