...
static uint8_t prkmcs_schedule_slot_operation(struct rtimer *slot_timer, rtimer_clock_t ref_time, rtimer_clock_t offset)
{
	...
	/*not necessary to calculate missing lot, Ling Wang, 11/29/2017*/
...
{			
	...
	/*Ling Wang, 12/1/2017*/
	network_time_set(0, 0);
	//network_time_set(0,0);
	/* Base station start to schedule slot, non-base station prepare to received synchronization information*/
	if (node_addr != BASE_STATION_ID)
	{
...
{		
	...
	{
		/*update time slot from realtimer, Ling Wang, 12/1/2017*/
		current_asn.ls4b = network_timeslot_now();
		printf("Slot %lu\r\n", current_asn.ls4b);
  ...
  */
	...
	{
		/* Before synchronize, should not schedule time slot, Ling Wang, 12/1/2017*/
		//SetChannel(control_channel);
		//start_rx();
	}
